...
test_key = "hotel"
X_trn, Y_trn, X_tst, Y_tst, vocabulary, vocabulary_inv = datasets.get_cross_domain_dataset(test_keys=[test_key])
Y_trn_o = Y_trn
Y_tst_o = Y_tst
...
for epoch in range(args.num_epochs):
    ...
    if (epoch + 1) > 20:
        checkpoint_path = os.path.join('save/cross_domain/'+test_key+'/model/', 'nlp-capsule'  + str(epoch + 1) + '.pth')
        torch.save(capsule_net.state_dict(), checkpoint_path)
        print("model saved to {}".format(checkpoint_path))

        checkpoint_path = os.path.join('save/cross_domain/'+test_key+'/loss/', 'loss' + str(epoch + 1) + '.csv')
