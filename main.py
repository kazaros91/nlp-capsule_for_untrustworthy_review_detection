...
for FOLD in range(1, 6):
    X_trn, Y_trn, Y_trn_o, X_tst, Y_tst, Y_tst_o, vocabulary, vocabulary_inv = data_helpers.load_data(fold=FOLD)
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) > 20:
            checkpoint_path = os.path.join('save/model/fold' + str(FOLD), 'nlp-capsule'  + str(epoch + 1) + '.pth')
            torch.save(capsule_net.state_dict(), checkpoint_path)
            print("model saved to {}".format(checkpoint_path))

            checkpoint_path = os.path.join('save/loss/fold'  + str(FOLD), 'loss' + str(epoch + 1) + '.csv')
