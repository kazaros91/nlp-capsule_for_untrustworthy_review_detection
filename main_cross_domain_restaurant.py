...
X_trn, Y_trn, X_tst, Y_tst, vocabulary, vocabulary_inv = datasets.get_cross_domain_restaurant()
...
for epoch in range(args.num_epochs):
    ...
    if (epoch + 1) > 20:
        checkpoint_path = os.path.join('save/model/cross_domain_doctor/', 'nlp-capsule'  + str(epoch + 1) + '.pth')
        torch.save(capsule_net.state_dict(), checkpoint_path)
        print("model saved to {}".format(checkpoint_path))

        checkpoint_path = os.path.join('save/loss/cross_domain_doctor/', 'nlp-capsule_loss' + str(epoch + 1) + '.csv')
